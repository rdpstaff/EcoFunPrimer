...
/**
 * Implements OligoFilter interface. HairpinTemp filter.
 *
 * @author gunturus, tift
 */
public class HairpinTempFilter implements OligoTempFilter{
    
    double max;
    
    public HairpinTempFilter(double max) {
        this.max = max;
    }
    /**
     * Checks an Oligo HairinTemp based off the max
     * 
     * @param oligo - oligo to be evaluated
     * @return boolean - true means the HairpinTemp is less than or equal to max
     */
    @Override
    public boolean check(Oligo oligo) {
        return oligo.getHairpinTm() <= max;
    }
